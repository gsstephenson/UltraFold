...
def plotArcRibbons(pairedNuc, ax, color, alpha=1.0):
    ...
    while i < len(pairedNuc):
        if pairedNuc[i] > i + 1:
            ...
            verts = [
                (outerPair[0], 0),
                (outerPair[0], -handleLengthFactor * outerRadius),
                (outerPair[0] + outerRadius - handleLengthFactor * outerRadius, -outerRadius),
                (outerPair[0] + outerRadius, -outerRadius),
                (outerPair[0] + outerRadius + handleLengthFactor * outerRadius, -outerRadius),
                (outerPair[1], -handleLengthFactor * outerRadius),
                (outerPair[1], 0),
                (innerPair[1], 0),
                (innerPair[1], -handleLengthFactor * innerRadius),
                (innerPair[0] + innerRadius + handleLengthFactor * innerRadius, -innerRadius),
                (innerPair[0] + innerRadius, -innerRadius),
                (innerPair[0] + innerRadius - handleLengthFactor * innerRadius, -innerRadius),
                (innerPair[0], -handleLengthFactor * innerRadius),
                (innerPair[0], 0),
                (outerPair[0], 0)
            ]

            for n in range(len(verts)):
                verts[n] = [verts[n][0], verts[n][1] - 1.2]
            codes = [
                Path.MOVETO,
...
def writePlot(outPath="arcs.pdf", title="", seq=["A"], pairedNucA=[], pairedNucB=[], alpha=0.5):
    ...
    fig = plot.figure(figsize=(figWidth, figHeight))
    ...
    if len(seq) <= 500:
        fontProp = mp.font_manager.FontProperties(family="monospace", style="normal",
                                                  weight="extra bold", size="4")
        for i in range(len(seq)):
            nuc = seq[i]
            if nuc == "T":
                nuc = "U"
            col = "black"
            plot.annotate(nuc, xy=(i + 0.5, ymax), fontproperties=fontProp,
                          color=col, annotation_clip=False, verticalalignment="top")
